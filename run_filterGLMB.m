function est = run_filterGLMB(model,meas,truth,r)
...
%filter parameters
filter.H_bth= 3;                    %requested number of birth components/hypotheses
filter.H_sur= 500;                 %requested number of surviving components/hypotheses
filter.H_upd= 500;                 %requested number of updated components/hypotheses
filter.H_max= 500;                 %cap on number of posterior components/hypotheses
...
for k=1:meas.K
    ...
    glmb_update= update(glmb_predict,model,filter,meas,k,truth,r);      H_posterior= length(glmb_update.w);
    ...
    [est.X{k},est.N(k),est.L{k}]= extract_estimates(glmb_update,model,k,truth);
...
for tabsidx=1:length(glmb_update.tt)
    wtemp_predict= compute_pS(model,glmb_update.tt{tabsidx}.x).*glmb_update.tt{tabsidx}.w(:); 
    xtemp_predict= gen_newstate_fn(model,glmb_update.tt{tabsidx}.x,'noise');      %particle prediction
    tt_survive{tabsidx}.x= xtemp_predict;                                                                                                                                   %samples for surviving track
    tt_survive{tabsidx}.w= wtemp_predict/sum(wtemp_predict);                                                                                                                %weights for surviving track
...
avps= zeros(length(glmb_update.tt),1);
for tabidx=1:length(glmb_update.tt)
    avps(tabidx)= glmb_update.tt{tabidx}.w(:)'*compute_pS(model,glmb_update.tt{tabidx}.x)+eps(0);
end
avqs= 1-avps;
...
for pidx=1:length(glmb_update.w)
    if glmb_update.n(pidx)==0 %no target means no deaths
        ...
        costv= avps(glmb_update.I{pidx})./avqs(glmb_update.I{pidx});                                                                %cost vector
        neglogcostv= -log(costv);                                                                                                   %negative log cost
        ...
        for hidx=1:length(nlcost)
            ...
            glmb_survive.w(runidx)= sum(log(avqs(glmb_update.I{pidx})))+log(glmb_update.w(pidx))-nlcost(hidx);                      %hypothesis/component weight
...
function glmb_update= update(glmb_predict,model,filter,meas,k,truth,r)
%create updated tracks (single target Bayes update)
% m=1;% 
%   m=truth.N(k);                                   %number of measurements
if k==1
    m=1;
else
 m =truth.N(k-1);
...
for emm= 1:m
    ...
    for tabidx= 1:length(glmb_predict.tt)
        ...
        w_temp= compute_pD(model,glmb_predict.tt{tabidx}.x).*glmb_predict.tt{tabidx}.w(:).*compute_likelihood_MUSIC(truth,model,glmb_predict.tt{tabidx}.x,EV,r,k,m)'; 
        x_temp= glmb_predict.tt{tabidx}.x;  %weight update for this track and this measuremnent      
...
%         glmb_update.w(pidx)= -model.lambda_c+sum(log(avqd(glmb_predict.I{pidx})))+log(glmb_predict.w(pidx));            %hypothesis/component weight
    glmb_update.w(pidx)= sum(log(avqd(glmb_predict.I{pidx})))+log(glmb_predict.w(pidx));            %hypothesis/component weight
   end
...
%             glmb_update.w(runidx)= -model.lambda_c+m*log(model.lambda_c*model.pdf_c)+log(glmb_predict.w(pidx));
            glmb_update.w(runidx)= log(glmb_predict.w(pidx));
...
%             costm= allcostm(glmb_predict.I{pidx},:)./(model.lambda_c*model.pdf_c*repmat(avqd(glmb_predict.I{pidx}),[1 m]));                     %cost matrix
            costm= allcostm(glmb_predict.I{pidx},:)./(repmat(avqd(glmb_predict.I{pidx}),[1 m])); 
...
%                 glmb_update.w(runidx)= -model.lambda_c+m*log(model.lambda_c*model.pdf_c)+sum(log(avqd(glmb_predict.I{pidx})))+log(glmb_predict.w(pidx))-nlcost(hidx);       %hypothesis/component weight
                glmb_update.w(runidx)= sum(log(avqd(glmb_predict.I{pidx})))+log(glmb_predict.w(pidx))-nlcost(hidx);       %hypothesis/component weight
...
%remove duplicate entries and clean track table
glmb_update= clean_update(glmb_update,filter);
end
...
function [X,N,L]=extract_estimates(glmb,model,k,truth)
...
for n=1:N
    X(:,n)= glmb.tt{glmb.I{idxcmp}(n)}.x*glmb.tt{glmb.I{idxcmp}(n)}.w(:);
